#ifndef MAKINA_CORE_ASPECTS_HIERARCHICAL_HPP_
#define MAKINA_CORE_ASPECTS_HIERARCHICAL_HPP_
...
public:
  ...
  const derived* get_child      (std::size_t index ) const
  {
    return index < children_.size() ? children_[index] : nullptr;
  }
        derived* get_child      (std::size_t index )
  {
    return index < children_.size() ? children_[index] : nullptr;
  }
