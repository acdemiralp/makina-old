#ifndef MAKINA_CORE_RENDERING_BACKENDS_OPENGL_FRAMEBUFFER_HPP_
#define MAKINA_CORE_RENDERING_BACKENDS_OPENGL_FRAMEBUFFER_HPP_
...
namespace gl
{
class MAKINA_EXPORT framebuffer
{
public:
...
MAKINA_EXPORT void set_color_mask  (const std::array<bool, 4>& mask);
MAKINA_EXPORT void set_color_mask  (GLuint index, const std::array<bool, 4>& mask);
MAKINA_EXPORT void set_depth_mask  (bool   mask );
MAKINA_EXPORT void set_stencil_mask(GLuint mask , GLenum face = GL_FRONT_AND_BACK);

// 17.4.3 Clearing the buffers.
MAKINA_EXPORT void set_clear_color  (const std::array<float, 4>& color);
MAKINA_EXPORT void set_clear_depth  (GLdouble   depth  );
MAKINA_EXPORT void set_clear_depth  (GLfloat    depth  );
MAKINA_EXPORT void set_clear_stencil(GLint      stencil);
MAKINA_EXPORT void clear            (GLbitfield buffer_bits = GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

// 18.2 Reading pixels.
MAKINA_EXPORT std::vector<GLuint> read_pixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type);

// 18.2.8 Final conversion.
MAKINA_EXPORT void clamp_read_color(GLenum clamp = GL_FIXED_ONLY);
}
}
