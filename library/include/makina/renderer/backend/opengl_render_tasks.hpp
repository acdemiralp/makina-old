...
struct MAKINA_EXPORT upload_scene_task_data
{
  ...
  buffer_resource*        instance_attributes; // [0] transform index, [1] material index, [2] indices offset
  buffer_resource*        indices            ;
  buffer_resource*        transform          ;
...
MAKINA_EXPORT fg::render_task<upload_scene_task_data>* add_upload_scene_render_task(renderer* framegraph, render_target_resource* render_target);
MAKINA_EXPORT fg::render_task<phong_task_data>*        add_phong_render_task       (renderer* framegraph, render_target_resource* render_target);
}
