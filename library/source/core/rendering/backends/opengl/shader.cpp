#include <makina/core/rendering/backends/opengl/shader.hpp>
...
{
  return get_parameter(GL_COMPILE_STATUS);
}
...
std::string shader::source         () const
{
  ...
  glGetShaderSource(id_, result.size(), nullptr, &result[0]);
  return result;
}
std::string shader::info_log       () const
{
  ...
  glGetShaderInfoLog(id_, result.size(), nullptr, &result[0]);
  return result;
}
