#include <makina/core/rendering/backends/opengl/program.hpp>
...
std::string program::resource_name                            (GLenum interface, GLuint index) const
{
  ...
  glGetProgramResourceName(id_, interface, index, name.size(), &length, &name[0]);
  name.resize(length);
  return name;
}
...
std::vector<GLint> program::resource_parameters(GLenum interface, GLuint index, const std::vector<GLenum>& parameters) const
{
  ...
  glGetProgramResourceiv(id_, interface, index, parameters.size(), parameters.data(), result.size(), nullptr, result.data());
  return result;
}
...
std::string program::active_uniform_name(GLuint index) const
{
  ...
  glGetActiveUniformName(id_, index, result.size(), nullptr, &result[0]);
  return result;
}
...
std::tuple<std::string, GLenum, GLsizei> program::active_uniform(GLuint index) const
{
  ...
  glGetActiveUniform(id_, index, name.size(), nullptr, &size, &type, &name[0]);
  return result;
}
...
{
  return get_active_uniform_parameter(index, GL_UNIFORM_IS_ROW_MAJOR);
}
...
std::string program::active_uniform_block_name(GLuint index) const
{
  ...
  glGetActiveUniformBlockName(id_, index, result.size(), nullptr, &result[0]);
  return result;
}
...
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER);
}
bool    program::active_uniform_block_is_referenced_by_fragment_shader               (GLuint index) const
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER);
}
bool    program::active_uniform_block_is_referenced_by_compute_shader                (GLuint index) const
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_COMPUTE_SHADER);
}
bool    program::active_uniform_block_is_referenced_by_geometry_shader               (GLuint index) const
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_GEOMETRY_SHADER);
}
bool    program::active_uniform_block_is_referenced_by_tessellation_control_shader   (GLuint index) const
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_TESS_CONTROL_SHADER);
}
bool    program::active_uniform_block_is_referenced_by_tessellation_evaluation_shader(GLuint index) const
{
  return get_active_uniform_block_parameter(index, GL_UNIFORM_BLOCK_REFERENCED_BY_TESS_EVALUATION_SHADER);
}
...
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_VERTEX_SHADER);
}
bool    program::active_atomic_counter_buffer_is_referenced_by_fragment_shader               (GLuint index) const
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_FRAGMENT_SHADER);
}
bool    program::active_atomic_counter_buffer_is_referenced_by_compute_shader                (GLuint index) const
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_COMPUTE_SHADER);
}
bool    program::active_atomic_counter_buffer_is_referenced_by_geometry_shader               (GLuint index) const
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_GEOMETRY_SHADER);
}
bool    program::active_atomic_counter_buffer_is_referenced_by_tessellation_control_shader   (GLuint index) const
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_TESS_CONTROL_SHADER);
}
bool    program::active_atomic_counter_buffer_is_referenced_by_tessellation_evaluation_shader(GLuint index) const
{
  return get_active_atomic_counter_buffer_parameter(index, GL_ATOMIC_COUNTER_BUFFER_REFERENCED_BY_TESS_EVALUATION_SHADER);
}
...
{
  glProgramUniform1iv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform2iv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform3iv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform4iv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform1uiv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform2uiv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform3uiv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform4uiv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform1fv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform2fv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform3fv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform4fv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform1dv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform2dv(id_, location, value.size(), value.data());
}
...
{
  glProgramUniform3dv(id_, location, value.size(), value.data());
}
...
std::string         program::active_subroutine_name                               (GLenum shader_type, GLuint subroutine_index) const
{
  ...
  glGetActiveSubroutineName(id_, shader_type, subroutine_index, result.size(), nullptr, &result[0]);
  return result;
}
std::string         program::active_subroutine_uniform_name                       (GLenum shader_type, GLuint subroutine_index) const
{
  ...
  glGetActiveSubroutineUniformName(id_, shader_type, subroutine_index, result.size(), nullptr, &result[0]);
  return result;
}
...
{
  return get_parameter(GL_DELETE_STATUS);
}
bool    program::link_status                             () const
{
  return get_parameter(GL_LINK_STATUS);
}
bool    program::validation_status                       () const
{
  return get_parameter(GL_VALIDATE_STATUS);
}
...
{
  return get_parameter(GL_TESS_GEN_POINT_MODE);
}
bool    program::is_separable                            () const
{
  return get_parameter(GL_PROGRAM_SEPARABLE);
}
bool    program::is_binary_retrievable                   () const
{
  return get_parameter(GL_PROGRAM_BINARY_RETRIEVABLE_HINT);
}
...
std::vector<shader> program::attached_shaders() const
{
  ...
  glGetAttachedShaders(id_, shader_ids.size(), nullptr, shader_ids.data());
...
std::string program::info_log() const
{
  ...
  glGetProgramInfoLog(id_, result.size(), nullptr, &result[0]);
  return result;
}
...
std::tuple<std::string, GLenum, GLsizei> program::active_attribute(GLuint location) const
{
  ...
  glGetActiveAttrib(id_, location, name.size(), nullptr, &size, &type, &name[0]);
  return result;
}
...
void program::set_transform_feedback_varyings(const std::vector<std::string>& varyings, GLenum buffer_mode)
{
  ...
  glTransformFeedbackVaryings(id_, varyings.size(), varyings_c.data(), buffer_mode);
}
...
std::tuple<std::string, GLenum, GLsizei> program::transform_feedback_varying(GLuint index) const
{
  ...
  glGetTransformFeedbackVarying(id_, index, name.size(), nullptr, &size, &type, &name[0]);
  return result;
}
...
std::vector<GLint> program::get_active_uniform_parameters             (std::vector<GLuint> indices,     GLenum parameter) const
{
  ...
  glGetActiveUniformsiv(id_, indices.size(), indices.data(), parameter, result.data());
  return result;
}
