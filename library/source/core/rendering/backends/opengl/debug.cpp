#include <makina/core/rendering/backends/opengl/debug.hpp>
...
{
  glDebugMessageControl(source, type, severity, ids.size(), ids.data(), enabled);
}

void insert_debug_log(const debug_log& log)
{
  glDebugMessageInsert(log.source, log.type, log.id, log.severity, log.message.size(), log.message.data());
}
...
{
  glObjectPtrLabel(sync_object.id(), label.size(), label.data());
}
...
std::vector<debug_log> debug_logs(GLuint count)
{
  ...
  auto real_count = glGetDebugMessageLog(count, message_data.size(), &sources[0], &types[0], &ids[0], &severities[0], &message_sizes[0], &message_data[0]);
