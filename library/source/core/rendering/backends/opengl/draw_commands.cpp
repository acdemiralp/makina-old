#include <makina/core/rendering/backends/opengl/draw_commands.hpp>
...
{
  return glIsEnabled(!fixed_index ? GL_PRIMITIVE_RESTART : GL_PRIMITIVE_RESTART_FIXED_INDEX);
}
...
{
  glDrawArraysIndirect(mode, reinterpret_cast<const void*>(offset));
}
...
void multi_draw_arrays                                (GLenum mode, const std::vector<std::pair<GLint, GLsizei>>& offset_count_pairs)
{
  ...
  glMultiDrawArrays(mode, offsets.data(), counts.data(), offset_count_pairs.size());
}
void multi_draw_arrays_indirect                       (GLenum mode, const GLint offset, GLsizei draw_count, GLsizei stride)
{
  glMultiDrawArraysIndirect(mode, reinterpret_cast<const void*>(offset), draw_count, stride);
}
...
{
  glDrawElementsIndirect(mode, type, reinterpret_cast<const void*>(offset));
}
...
void multi_draw_elements                              (GLenum mode, GLenum type, const std::vector<std::pair<GLint, GLsizei>>& offset_count_pairs)
{
  ...
  glMultiDrawElements(mode, counts.data(), type, reinterpret_cast<void**>(offsets.data()), offset_count_pairs.size());
}
void multi_draw_elements_indirect                     (GLenum mode, GLenum type, const GLint offset, GLsizei draw_count, GLsizei stride)
{
  glMultiDrawElementsIndirect(mode, type, reinterpret_cast<const void*>(offset), draw_count, stride);
}
...
void multi_draw_elements_base_vertex                  (GLenum mode, GLenum type, const std::vector<std::tuple<GLint, GLsizei, GLint>>& offset_count_base_vertex_triplets)
{
  ...
  {
    offsets      .push_back(std::get<0>(triplet));
    counts       .push_back(std::get<1>(triplet));
    base_vertices.push_back(std::get<2>(triplet));
  }
  glMultiDrawElementsBaseVertex(mode, counts.data(), type, reinterpret_cast<void**>(offsets.data()), offset_count_base_vertex_triplets.size(), base_vertices.data());
}
}
}