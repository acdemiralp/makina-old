#include <makina/core/rendering/backends/opengl/framebuffer.hpp>
...
{
  return get_parameter(GL_FRAMEBUFFER_DEFAULT_FIXED_SAMPLE_LOCATIONS);
}
...
{
  return get_parameter(GL_DOUBLEBUFFER);
}
...
{
  return get_parameter(GL_STEREO);
}
...
{
  return get_attachment_parameter(attachment, GL_FRAMEBUFFER_ATTACHMENT_LAYERED);
}
...
{
  glInvalidateNamedFramebufferSubData(id_, attachments.size(), attachments.data(), x, y, width, height);
}
void framebuffer::invalidate         (const std::vector<GLenum>& attachments)
{
  glInvalidateNamedFramebufferData(id_, attachments.size(), attachments.data());
}
