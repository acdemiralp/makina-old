...
void input_system::update (frame_timer::duration delta, scene* scene)
{
  ...
  for(auto& entity : entities)
  {
    ...
    if (std::find(controller_cache_.begin(), controller_cache_.end(), controller) == controller_cache_.end())
    {
      ...
      if(controller->on_mouse_move   ) on_mouse_move_delta.connect([=] (std::array<std::size_t, 2> value ) { controller->on_mouse_move   (transform, value ); });
      if(controller->on_mouse_wheel  ) on_mouse_wheel     .connect([=] (std::array<std::size_t, 2> value ) { controller->on_mouse_wheel  (transform, value ); });
