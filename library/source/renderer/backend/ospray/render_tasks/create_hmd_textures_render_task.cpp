...
fg::render_task<create_hmd_textures_task_data>* add_create_hmd_textures_render_task(renderer* renderer, di::hmd* hmd)
{
  ...
  const auto left  = renderer->add_retained_resource<framebuffer::description, framebuffer>(
    "Left Eye Framebuffer" , framebuffer::description { size, OSP_FB_SRGBA, OSP_FB_COLOR | OSP_FB_DEPTH | OSP_FB_ACCUM });
  const auto right = renderer->add_retained_resource<framebuffer::description, framebuffer>(
    "Right Eye Framebuffer", framebuffer::description { size, OSP_FB_SRGBA, OSP_FB_COLOR | OSP_FB_DEPTH | OSP_FB_ACCUM });
