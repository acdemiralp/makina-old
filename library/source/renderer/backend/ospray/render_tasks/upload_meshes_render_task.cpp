...
{
  return renderer->add_render_task<upload_meshes_task_data>(
    "Ospray Upload Meshes Pass",
    ...
    [=] (const upload_meshes_task_data& data)
    {
      ...
      for (auto entity : scene->entities<transform, point_render>())
      {
        ...
        std::transform(
          ...
          [ ] (const glm::vec3& iteratee) { return glm::vec2 {iteratee.x, iteratee.y}; });

        const auto geometry            = std::make_shared<::ospray::cpp::Geometry>("triangles");
        const auto vertices            = std::make_shared<::ospray::cpp::Data>(mesh_render->mesh->vertices    .size()    , OSP_FLOAT3, mesh_render->mesh->vertices    .data()); vertices           ->commit();
        const auto normals             = std::make_shared<::ospray::cpp::Data>(mesh_render->mesh->normals     .size()    , OSP_FLOAT3, mesh_render->mesh->normals     .data()); normals            ->commit();
        ...
        geometry->set("vertex.normal"  , *normals            );
        geometry->set("vertex.color"   , *colors             );
        geometry->set("vertex.texcoord", *texture_coordinates);
