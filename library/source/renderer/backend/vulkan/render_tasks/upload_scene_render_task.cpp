...
namespace vulkan
{
void load_image(mak::image* source, std::shared_ptr<vkhlf::Buffer> intermediate, const std::shared_ptr<vkhlf::Image> target, const std::size_t offset)
{
    ...
    [=] (const upload_scene_task_data& data)
    {
      ...
      for (auto i = 0; i < mesh_render_entities.size(); ++i)
      {
        ...
        if  (pbr_material)
        {
          ...
          {
            load_image(pbr_material->metallicity_image      .get(), *data.intermediates->actual(), *data.images->actual(), texture_offset);
            metallicity_offset = texture_offset++;
          }
          if (pbr_material->roughness_image)
          {
            load_image(pbr_material->roughness_image        .get(), *data.intermediates->actual(), *data.images->actual(), texture_offset);
        ...
        for(auto j = 0; j < mesh_vertices.size(); ++j)
          vertices.push_back(vertex{glm::vec4(mesh_vertices[j], 1.0f), glm::vec4(mesh_normals[j], 0.0f), texture_coordinate_scale * mesh_texture_coordinates[j], {i, i}});
      ...
      (*data.vertices->actual())->update<vertex>       (0, {static_cast<std::uint32_t>(vertices.size()), vertices.data()}, command_buffer);
      ...
      (*data.transforms ->actual())->update<glsl_transform>                (sizeof glm::vec4, {static_cast<std::uint32_t>(transforms     .size()), transforms     .data()}, command_buffer);
      pbr_materials.size() > 0 
      ? (*data.materials->actual())->update<glsl_physically_based_material>(sizeof glm::vec4, {static_cast<std::uint32_t>(pbr_materials  .size()), pbr_materials  .data()}, command_buffer)
      : (*data.materials->actual())->update<glsl_phong_material>           (sizeof glm::vec4, {static_cast<std::uint32_t>(phong_materials.size()), phong_materials.data()}, command_buffer);
      (*data.cameras    ->actual())->update<glsl_camera>                   (sizeof glm::vec4, {static_cast<std::uint32_t>(cameras        .size()), cameras        .data()}, command_buffer);
      (*data.lights     ->actual())->update<glsl_light>                    (sizeof glm::vec4, {static_cast<std::uint32_t>(lights         .size()), lights         .data()}, command_buffer);
