...
void opengl::make_default_framegraph(engine* engine, di::opengl_window* main_window)
{
  ...
  if (vr_system)
  {
    ...
    const auto left_pbr_render_task            = add_physically_based_shading_render_task(renderer,               left_texture , upload_common_render_task->data(), upload_meshes_task_data, "hmd_left_camera" );
    ...
    const auto right_pbr_render_task           = add_physically_based_shading_render_task(renderer,               right_texture, upload_common_render_task->data(), upload_meshes_task_data, "hmd_right_camera");
  ...
  {                                                                          
    ...
    const auto pbr_render_task                 = add_physically_based_shading_render_task(renderer,               backbuffer, upload_common_render_task->data(), upload_meshes_task_data);
