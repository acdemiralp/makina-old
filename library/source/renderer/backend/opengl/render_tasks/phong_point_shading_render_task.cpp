...
namespace opengl
{
fg::render_task<phong_point_shading_task_data>* add_phong_point_shading_render_task(
  renderer*                      framegraph , 
...
{
  const auto retained_program = framegraph->add_retained_resource<graphics_program_resource::description_type, program>("Phong Point Shading Program", program::graphics_description
  {
    glsl::point_vertex_shader,
    glsl::point_phong_fragment_shader
  });

  return framegraph->add_render_task<phong_point_shading_task_data>(
    "Phong Point Shading Pass",
    [&] (      phong_point_shading_task_data& data, fg::render_task_builder& builder)
    {
      ...
      data.vertex_array        = builder.create<vertex_array_resource>("Phong Point Shading Vertex Array", vertex_array::description
      {
        { 
    ...
    [=] (const phong_point_shading_task_data& data)
    {
    ...
    gl::multi_draw_arrays_indirect(GL_POINTS, 0, data.parameter_map->actual()->get<GLsizei>("draw_count"));
      ...
      gl::print_error("Error in Phong Point Shading Pass");
    });
}
}
}
